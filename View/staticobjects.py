...
class View_players(__Object_base):
    ...
    def draw(self, screen):
        for player in self.model.players:
            # player itself
            status = 0 if player.direction == 'right' else 1
            ...
            # blood
            pg.draw.rect(screen, Const.HP_BAR_COLOR[1], [player.left, player.top+10, player.width*player.blood/Const.PLAYER_FULL_BLOOD, 5])
            # empty hp bar
            pg.draw.rect(screen, Const.HP_BAR_COLOR[0], [player.left, player.top+10, player.width, 5], 2)